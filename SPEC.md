...
| `LATEX-COMMAND-UNKNOWN` | A text field or a name uses a LaTeX command the converter has no rule for (`labdata.parsers.latex.unknown_commands()`). The command is dropped and a braced argument after it is kept as plain text, so no raw LaTeX reaches the document. Math is not searched. Reported once per field and command. A warning. |
...
| `PROJECTS-NOT-A-LIST`, `PROJECTS-FIELD-MISSING`, `PROJECTS-ID-DUPLICATE` | The same three conditions for `projects_file`, whose required fields are `id` and `title`. The same classes. |
...
- **The person and project strings supplied in YAML.**
  ...
  of any kind — they check that the required fields are there and that a
