...
| `names.equal_contribution_normalized` | `Brown$^{*}$*` (the marker twice) and `Kim{$^{*}$}` (the marker in a brace group of its own) | Both come off: nothing is left in the name and the author is marked and resolves | `tests/corpus/valid/names.bib` | `test_valid_corpus.py::test_equal_contribution` | pass |
