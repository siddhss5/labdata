...
| `identity.grouping_initials` | `Quinn, Q.` beside `Quinn, Quentin`, and five more pairs: a surname particle, two initials, a hyphenated family name, a name outside ASCII, and a lineage suffix that agrees | Two keys each, which differ — the instability is intended, not a merge — and an `ID-GROUPING-INITIALS-AMBIGUOUS` warning naming both. Decided on the structured parts, so none of the four shapes is missed. Not an error | `tests/corpus/valid/names.bib` | `test_valid_corpus.py::test_an_initials_only_key_that_could_be_a_fuller_one_is_reported` | pass |
