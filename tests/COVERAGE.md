...
| `structure.duplicate_key_file` | The same citation key twice in one file | Stable `E-BIB-DUPLICATE-KEY` error names the file, key and `citation_key` field | `tests/corpus/invalid/duplicate_key_same_file/dup.bib` | `test_invalid_corpus.py::test_locates` | pass |
| `structure.duplicate_key_across` | The same citation key in two files | Stable `E-BIB-DUPLICATE-KEY` error names both files, keys and `citation_key` field | `tests/corpus/invalid/duplicate_key_across_files/first.bib` | `test_invalid_corpus.py::test_locates` | pass |
