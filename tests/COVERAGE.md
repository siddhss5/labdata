...
| `identity.grouping_distinct` | Four names each differing from an initials-only name above in exactly one of the things the check compares: the particle, the family name, the second initial, and the second half of a hyphenated given name | None of them is reported as a name an initials-only key could be. The set of pairs the corpus produces is asserted whole, so a check comparing one thing less adds a pair and one comparing more drops one | `tests/corpus/valid/names.bib` | `test_valid_corpus.py::test_the_initials_warnings_are_exactly_these_pairs` | pass |
...
| `identity.grouping_initials` | `Quinn, Q.` beside `Quinn, Quentin`, and four more pairs: a surname particle, two initials, a hyphenated family name, and a name outside ASCII | Two keys each, which differ — the instability is intended, not a merge — and an `ID-GROUPING-INITIALS-AMBIGUOUS` warning naming both. Decided on the structured parts, so none of the four shapes is missed. Not an error | `tests/corpus/valid/names.bib` | `test_valid_corpus.py::test_an_initials_only_key_that_could_be_a_fuller_one_is_reported` | pass |
