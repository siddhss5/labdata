...
| `identity.collaborator_alias_is_member` | A `collaborators_file` alias, `A. Adams`, that a lab member also declares | A `RESOLVE-COLLABORATOR-ALIAS-IS-MEMBER` warning naming the file, the collaborator and the alias; the member keeps the spelling | `tests/corpus/valid/collaborators.yaml` | `test_config_cli.py::test_collaborator_alias_that_is_a_member_is_reported` | pass |
