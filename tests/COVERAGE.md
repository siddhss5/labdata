...
| `fields.demo_every_field` | Every field of every demo entry except `project`, which `fields.project` covers | Reaches its property with the input's value; `eprint` under the scheme `archivePrefix` names, lower-cased; `url` as a link with `origin: input` | `examples/demo/lab.yaml` | `test_consumer_probes.py::test_demo_field_reaches_the_document` | pass |
